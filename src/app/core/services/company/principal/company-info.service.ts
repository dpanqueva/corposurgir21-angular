...
export class CompanyInfoService {
  ...
  getCompanyInfoAll(): Observable<CompanyInfo[]> {
    ...
    .pipe(
      catchError(e => this.errorsApiGenerate(e))
    );
  }
  ...
  private errorsApiGenerate(e:any){
    ...
    if (e.status == 404) {
      return this.messageService.errorMessage(environment.mensaje_no_encontrado);
    }
    if (e.status == 500) {
     return this.messageService.errorMessage(environment.mensaje_internal_error);
    }
